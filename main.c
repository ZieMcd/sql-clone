 ...
 */
const uint32_t LEAF_NODE_NUMBER_CELLS_SIZE = sizeof(uint32_t);
const uint32_t LEAF_NODE_NUMBER_CELLS_OFFSET = COMMON_NODE_HEADER_SIZE;
const uint32_t LEAF_NODE_HEADER_SIZE =
    COMMON_NODE_HEADER_SIZE + LEAF_NODE_NUMBER_CELLS_SIZE;
...
void* get_page(Pager* pager, uint32_t page_num) {
  ...
  if (pager->pages[page_num] == NULL) {
    // Cache miss. Allocate memeory and load file
...
uint32_t* leaf_node_num_cells(void* node) {
  return node + LEAF_NODE_NUMBER_CELLS_OFFSET;
}
...
void set_node_type(void* node, NodeType NodeType) {
  ...
  *((uint8_t*)(node + NODE_TYPE_OFFSET)) = value;
  // node+NODE_TYPE_OFFSET = NodeType;
}
...
Cursor* leaf_node_find(Table* table, uint32_t page_num, uint32_t key) {
  ...
  while (one_past_max_index != min_index) {
    ...
    uint32_t key_at_index = *leaf_node_cell_key(node, index);
    if (key == key_at_index) {
...
Cursor* find_tabe(Table* table, uint32_t key) {
...
Table* db_open(const char* filename) {
  ...
  if (pager->num_pages == 0) {
    // New Db, initialize page 0 as root node
...
void* cursor_value(Cursor* cursor) {
  ...
  return leaf_node_cell_value(page, cursor->cell_num);
}
...
void leaf_node_insert(Cursor* cursor, uint32_t key, Row* value) {
  ...
  if (num_cells >= LEAF_NODE_MAX_CELLS) {
    printf("Need to implement slitting of cells \n");
    exit(EXIT_FAILURE);
  }
  ...
  *(leaf_node_cell_key(node, cursor->cell_num)) = key;
  serialize_row(value, leaf_node_cell_value(node, cursor->cell_num));
}
...
ExecuteResult execute_insert(Statement* statement, Table* table) {
  ...
  Cursor* cursor = find_tabe(table, key_to_insert);

  if (cursor->cell_num < num_cells) {
    uint32_t key_at_index = *leaf_node_cell_key(node, cursor->cell_num);
...
ExecuteResult exuce_statement(Statement* statement, Table* table) {
...
int main(int argc, char* argv[]) {
  ...
  while (true) {
    ...
    switch (exuce_statement(&statement, table)) {
    case EXCUTE_SUCCESS:
      printf("Executed.\n");
      break;
